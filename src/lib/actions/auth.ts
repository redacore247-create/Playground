...
const RegisterSchema = z.object({
  ...
  username: z.string().min(3).max(24).regex(/^[a-z0-9_]+$/, 'Only lowercase letters, numbers, underscores'),
...
export async function registerWithEmail(formData: FormData): Promise<AuthResult> {
  ...
  const { data: existing } = await supabase
    .from('profiles').select('id').eq('username', parsed.data.username).maybeSingle()
  if (existing) return { success: false, error: 'Username is already taken' }

  const { data, error } = await supabase.auth.signUp({
    email: parsed.data.email,
    password: parsed.data.password,
    options: { data: { username: parsed.data.username, full_name: parsed.data.display_name ?? parsed.data.username } },
  })
  if (error) {
  ...
  if (data.user && parsed.data.display_name) {
    await supabase.from('profiles').update({ display_name: parsed.data.display_name }).eq('id', data.user.id)
  }
  redirect('/')
}
...
export async function loginWithGoogle(): Promise<AuthResult> {
  ...
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider: 'google',
    options: { redirectTo: `${process.env.NEXT_PUBLIC_APP_URL}/auth/callback`, queryParams: { prompt: 'select_account' } },
  })
...
export async function updateProfile(data: z.infer<typeof ProfileUpdateSchema>): Promise<AuthResult> {
  const supabase = await createClient()
...
export async function toggleFollow(targetUserId: string): Promise<{ success: boolean; following: boolean; error?: string }> {
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  ...
  const { data: existing } = await supabase
    .from('follows').select('follower_id').eq('follower_id', user.id).eq('following_id', targetUserId).maybeSingle()
